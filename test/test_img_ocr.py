...
class TestMain(unittest.TestCase):
    ...
    def test_subarea_ocr(self):
        """
        识别图片的子区域
        """
        logger.info(f'Begin picture subarea ocr...')
        infile = '../pic/B3.png'
        outfile = '../output/B3.sub.png'

        image = cv2.imread(infile)
        ocr = PaddleOCR()
        subregion = image[350:550, 1100:1800]    # 第一参数为行，第二参数为列，设定子区域

        results = ocr.ocr(subregion)             # 识别
        cv2.imwrite(outfile, subregion)          # 保存区域文件
        # 1. 打开已有的图片文件
        input_image = Image.open(outfile)  # 将路径替换为你的输入图片文件路径
        # 2. 标注图像
        # 创建一个绘图对象
        draw = ImageDraw.Draw(input_image)
        # 在图片上绘制矩形，文本，置信度
